    ...
    public class ControllerWalker : CSharpSyntaxWalker
    {
        ...
        public override void VisitMethodDeclaration(MethodDeclarationSyntax node)
        {
            if (node.Parent is ClassDeclarationSyntax)
            {
                ...
                if (Config.UseAttribute || nodeClass.IsController())
                {
                    Controller controller = new Controller(nodeClass);
                    ControllerAction action = new(node.Identifier.ToString());

                    bool validAttribute = !Config.UseAttribute || node.AttributeLists.ContainsAttribute(Config.OnlyWhenAttributed);
                    if (validAttribute)
                    {
